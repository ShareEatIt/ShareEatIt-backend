...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    @Query("SELECT c FROM ChatRoom c " +
            "JOIN FETCH c.participation " +  // 성능 최적화 & N+1 문제 해결 위해 join fetch  사용
            "WHERE (c.participation.giver.id = :memberId AND c.participation.isGiverInChat = true) " +
            "OR (c.participation.receiver.id = :memberId AND c.participation.isReceiverInChat = true)")
