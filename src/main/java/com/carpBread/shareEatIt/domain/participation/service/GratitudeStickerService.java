...
public class GratitudeStickerService {
    ...
    public GratitudeResponseDto createGratitudeSticker(Long postId, Member member, GratitudeType gratitudeType){
        ...
        if (postId == null || member == null || gratitudeType == null) {
            throw new CustomException(CAN_NOT_BE_NULL, "필수 입력 값이 누락되었습니다.", "GratitudeStickerService" + postId, "/gratitudeSticker", PARTICIPATION);
        }

        // SharingPost 객체 찾기
        SharingPost post = sharingPostRepository.findById(postId)
                .orElseThrow(() -> new CustomException(NOT_FOUND_SHARINGPOST, "해당 Id의 SharingPost를 찾을 수 없습니다.", "GratitudeStickerService", "/gratitudeStickers/" + postId,  PARTICIPATION));

        // 검증1 : 해당 나눔글의 상태가 COMPLETED인지 확인
        if (!post.getStatus().equals(PostStatus.COMPLETED)){
            throw new CustomException(NOT_COMPLETED_SHARINGPOST, "완료되지 않은 나눔입니다.","GratitudeStickerService", "/gratitudeStickers/" + postId, PARTICIPATION);
        }
        ...
        if (!member.getId().equals(participation.getReceiver().getId())){
            throw new CustomException(NOT_RECEIVER_OF_SHARINGPOST, "해당 나눔을 받은 멤버가 아닙니다.", "GratitudeStickerService", "/gratitudeStickers/" + postId, PARTICIPATION);
        }

        // 검증 5 : 이미 고마움을 남긴 경우
        if (gratitudeStickerRepository.existsByParticipationId(participation.getId())){
            throw new CustomException(ALREADY_EXISTS_GRATITUDESTICKER, "이미 고마움을 남긴 나눔입니다.", "GratitudeStickerService",  "/gratitudeStickers/" + postId, PARTICIPATION);
        }
    ...
    public Participation findSingleParticipation(Long postId) {
        ...
        if (participationList.isEmpty()) { // 검증2 : 찾지 못한 경우
            throw new CustomException(NOT_FOUND_PARTICIPATION, "해당 나눔글의 완료된 참여정보를 불러올 수 없습니다.", "GratitudeStickerService", "/gratitudeStickers/" + postId, PARTICIPATION);
        }

        if (participationList.size() > 1) { //검증3 : COMPLETED인 참여가 여러개인 경우
            throw new CustomException(MULTIPLE_COMPLETED_PARTICIPATIONS, "같은 나눔글에 대해 참여 상태가 COMPLETED인 참여 객체가 여러개입니다.", "GratitudeStickerService", "/gratitudeStickers/" + postId, PARTICIPATION);
        }
    ...
    public GratitudeResponseDto updateGratitudeStickers(Long gratitudeStickerId, Member member, GratitudeType gratitudeType) {
        ...
        if (gratitudeStickerId == null || member == null || gratitudeType == null) {
            throw new CustomException(CAN_NOT_BE_NULL, "필수 입력 값이 누락되었습니다.", "GratitudeStickerService", "/gratitudeSticker" +gratitudeStickerId, PARTICIPATION);
        }

        // gratitudeSticker 객체 찾기
        GratitudeSticker gratitudeSticker = gratitudeStickerRepository.findById(gratitudeStickerId)
                .orElseThrow(() -> new CustomException(NOT_FOUND_GRATITUDESTICKER, "해당 Id의 gratitudeSticker를 찾을 수 없습니다.", "GratitudeStickerService", "/gratitudeSticker/" +gratitudeStickerId, PARTICIPATION));
        ...
        if (reviewer == null || !member.getId().equals(reviewer.getId())){
            throw new CustomException(NOT_REVIEWER_OF_SHARINGPOST, "해당 나눔을 받은 멤버가 아닙니다.", "GratitudeStickerService", "/gratitudeSticker/" +gratitudeStickerId, PARTICIPATION);
        }
